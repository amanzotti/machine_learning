...
sigma_vector = np.linspace(0.001, 0.1, 20)
sigma_test_error = []

for n, sigma in enumerate(sigma_vector):
    ...
    for i in range(examples):
        for j in range(examples):
            K[i, j] = gaussian_kernel(X_train[i], X_train[j], sigma=sigma)
...
# K = np.load('kernel_linear_2000.npy')
    ...
    for test_num in xrange(0, examples):
        ...
        for a, sv_y_loop, sv_loop in zip(alpha, sv_y, sv):
            s += a * sv_y_loop * gaussian_kernel(X_test[test_num], sv_loop, sigma=sigma)
            # print s
        label.append(np.sign(s))

    sigma_test_error.append(np.mean(y_test == label))

np.savetxt('sigma_test_{}.txt'.format(time.time()), np.vstack((sigma_vector,sigma_test_error)).T )
